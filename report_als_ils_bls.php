...
$query_primary = sprintf("SELECT casename FROM call_log WHERE tfrtime > %s AND tfrtime < %s AND carelevel = 3", $settime, $nextmo);
...
mysql_select_db($database_adhocConn, $adhocConn);
$query_transfer = sprintf("SELECT casename FROM call_log WHERE tfrtime > %s AND tfrtime < %s AND carelevel = 2", $settime, $nextmo);
...
mysql_select_db($database_adhocConn, $adhocConn);
$query_bls = sprintf("SELECT casename FROM call_log WHERE tfrtime > %s AND tfrtime < %s AND carelevel = 1", $settime, $nextmo);
$bls = mysql_query($query_bls, $adhocConn) or die(mysql_error());
$row_bls = mysql_fetch_assoc($bls);
$totalRows_bls = mysql_num_rows($bls);
?>

<?php require_once('inc_before.php'); ?>
              <p>Care Level Summary</p>
              <table border="0" align="center" cellpadding="10" cellspacing="0">
                <tr>
                  <td><div align="left"><a href="report_als_ils_bls.php?settime=<?php echo $prevmo; ?>" class="attention">previous month</a></div></td>
                  <td><div align="center"><?php echo date("F Y", $settime); ?></div></td>
                  <td><div align="right"><a href="report_als_ils_bls.php?settime=<?php echo $nextmo; ?>" class="attention">next month</a></div></td>
                </tr>
              </table>
              <br />
              <table border="0" align="center" cellpadding="5" cellspacing="0">
                <tr>
                  <td><div align="right">ALS:</div></td>
                  <td><div align="left"><?php echo $totalRows_primary; ?></div></td>
                  </tr>
                <tr>
                  <td><div align="right">ILS:</div></td>
                  <td><div align="left"><?php echo $totalRows_transfer; ?></div></td>
                  </tr>
                <tr>
                  <td><div align="right">BLS:</div></td>
                  <td><div align="left"><?php echo $totalRows_bls; ?></div></td>
                  </tr>
              </table>
