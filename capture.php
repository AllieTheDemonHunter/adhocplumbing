...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
 ...
 $updateSQL = sprintf("UPDATE call_log SET service=%s, geyser=%s, jobcard_no=%s, ins_receipt_no=%s, actual_job=%s, patch_work=%s, cons_damage=%s, notes=%s, cons_notes=%s, capturer=%s, job_complete=%s,call_status=6 WHERE logID=%s",
                    GetSQLValueString($_POST['service'], "text"),
					GetSQLValueString($_POST['geyser'], "text"),
                    GetSQLValueString($_POST['jobcard_no'], "text"),
                    GetSQLValueString($_POST['ins_receipt_no'], "text"),
                    GetSQLValueString($condition, "text"),
                    GetSQLValueString($_POST['notes'], "text"),
					GetSQLValueString($_POST['cons_notes'], "text"),
                    GetSQLValueString($_POST['capturer'], "int"),
					GetSQLValueString($_POST['patch_work'], "text"),
					GetSQLValueString($_POST['job_complete'], "int"),
					GetSQLValueString($_POST['cons_damage'], "text"),
         ...
         <option value="No Service - Other" <?php if (!(strcmp("No Service", $row_call['service']))) {echo "selected=\"selected\"";} ?>>No Service - Other</option>
                  ...
                  <tr valign="baseline">
                    ...
                    <td align="right" valign="top" nowrap="nowrap">Patchwork:</td>
                  	   <td><select name="patch_work" class="maintext">
						 <option value="Yes" <?php if (!(strcmp("Yes", $row_call['patch_work']))) {echo "selected=\"selected\"";} ?>>Yes</option>
						 <option value="No" <?php if (!(strcmp("No", $row_call['patch_work']))) {echo "selected=\"selected\"";} ?>>No</option>
					   </select></td>
                    </tr>
                     <tr valign="baseline">
                   ...
                   <tr valign="baseline">
                    ...
                    <td><select name="cons_damage" class="maintext">
						 <option value="Yes" <?php if (!(strcmp("Yes", $row_call['cons_damage']))) {echo "selected=\"selected\"";} ?>>Yes</option>
						 <option value="No" <?php if (!(strcmp("No", $row_call['cons_damage']))) {echo "selected=\"selected\"";} ?>>No</option>
					   </select></td>
                    </tr>
					<tr valign="baseline">
						<td align="right" valign="top" nowrap="nowrap">Note:</td>
						<td><textarea name="cons_notes" cols="40" rows="3" class="maintext"><?php echo htmlentities($row_call['cons_notes'], ENT_COMPAT, 'utf-8'); ?></textarea></td>
                    </tr>
                    <tr valign="baseline">
					  <td>Geyser:</td>			
					   <td><select name="geyser" class="maintext">
                        <option value="N/A" <?php if (!(strcmp("N/A", $row_call['geyser']))) {echo "selected=\"selected\"";} ?>>N/A</option>
						 <option value="Office(Off Site)" <?php if (!(strcmp("Office", $row_call['geyser']))) {echo "selected=\"selected\"";} ?>>Office (Off Site)</option>
						 <option value="Client(On Site)" <?php if (!(strcmp("Client", $row_call['geyser']))) {echo "selected=\"selected\"";} ?>>Client (On Site)</option>
					   </select></td>
					</tr>
                  <tr valign="baseline">
