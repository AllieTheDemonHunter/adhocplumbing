...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
	$updateSQL = sprintf("UPDATE call_log SET dispatcher=%s, vehicle1=%s, notes=%s, call_status=%s WHERE logID=%s",
	...
	if (($_POST['vehicle1'] != "GIVE AWAY")) {
		...
		$v1name = $row_vehicle['regno'];
		$cellno = $row_vehicle['cellno'];
		mysql_free_result($vehicle);
		$msg1 = "DESPATCH: " . $_POST['address'] . " vehicle " . $v1name . ". " . $_POST['notes'];
	...
	$updateGoTo = "confirm_despatch.php?caseno=" . $_POST['logID'];
...
do {  
?>
                          <option value="<?php echo $row_v1['vehicleID']?>" <?php if (!(strcmp($row_v1['vehicleID'], htmlentities($row_call['vehicle1'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>><?php echo $row_v1['vehicle']?></option>
                          <?php
...
?>
                  ...
                  <input type="hidden" name="address" value="<?php echo htmlentities($row_call['address'], ENT_COMPAT, 'utf-8'); ?>" />
                  <input type="hidden" name="mine" value="<?php echo htmlentities($row_call['mine'], ENT_COMPAT, 'utf-8'); ?>" />
                  <input type="hidden" name="call_status" value="2" />
