...
<table width="100%" border="1" cellspacing="0" cellpadding="3">
  <tr>
    ...
    <?php do { 
		...
		$display = 0;
		if ($_POST['searchby'] == "clientsurname") {
			similar_text(strtoupper($row_recent['surname']), strtoupper($_POST['searchfor']), $percent2);
			if ((strpos(strtoupper($row_recent['surname']), strtoupper($_POST['searchfor']))) || ($percent2 > 80)) {
				$display = 1;
			}
		}
		if ($_POST['searchby'] == "contact") {
			similar_text(strtoupper($row_recent['caller']), strtoupper($_POST['searchfor']), $percent2);
			if ((strpos(strtoupper($row_recent['caller']), strtoupper($_POST['searchfor']))) || ($percent2 > 80)) {
				$display = 1;
			}
		}
		if ($_POST['searchby'] == "contacttel") {
			similar_text(strtoupper($row_recent['telno1']), strtoupper($_POST['searchfor']), $percent2);
			if ((strpos(strtoupper($row_recent['telno1']), strtoupper($_POST['searchfor']))) || ($percent2 > 80)) {
				$display = 1;
			}
			similar_text(strtoupper($row_recent['telno2']), strtoupper($_POST['searchfor']), $percent2);
			if ((strpos(strtoupper($row_recent['telno2']), strtoupper($_POST['searchfor']))) || ($percent2 > 80)) {
				$display = 1;
			}
			similar_text(strtoupper($row_recent['telno3']), strtoupper($_POST['searchfor']), $percent2);
			if ((strpos(strtoupper($row_recent['telno3']), strtoupper($_POST['searchfor']))) || ($percent2 > 80)) {
				$display = 1;
			}
		}
		if ($display == 1) {
...
?>
<?php if ($_POST['searchby'] != "caseno") { ?><br />
    <form id="client" name="client" method="post" action="log_quote_search.php"><div align="center">Search by 
    <select name="searchby" class="maintext" id="searchby">
        <option value="clientsurname" <?php if (!(strcmp("clientsurname", $_POST['searchby']))) {echo "selected=\"selected\"";} ?>>Client Surname</option>
      <option value="contact" <?php if (!(strcmp("contact", $_POST['searchby']))) {echo "selected=\"selected\"";} ?>>Contact Person</option>
      <option value="contacttel" <?php if (!(strcmp("contacttel", $_POST['searchby']))) {echo "selected=\"selected\"";} ?>>Contact Tel No</option>
    </select>
    ======> Search for:
    ...
    <input name="searchfor" type="text" class="maintext" id="searchfor" size="15" value="<?php echo $_POST['searchfor']; ?>" />
    <input name="button" type="submit" class="maintext" id="button" value="Search for non-exact matches" />
    </div>
    </form>
<?php } ?>
