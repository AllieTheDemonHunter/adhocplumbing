  ...
  return $theValue;
    ...
    if (((strrchr($_FILES["file"]["name"],".") == ".doc") || (strrchr($_FILES["file"]["name"],".") == ".docx") || (strrchr($_FILES["file"]["name"],".") == ".jpg") || (strrchr($_FILES["file"]["name"],".") == ".tif") || (strrchr($_FILES["file"]["name"],".") == ".pdf")) && ($_FILES["file"]["size"] < 1024000)) {
        ...
        if ($_FILES["file"]["error"] > 0) {
			...
			$filename = "docs/".$new_filename.strrchr($_FILES["file"]["name"],".");
			
			// insert into DB
			$insertSQL = sprintf("INSERT INTO pdfs (pdfname, case_no) VALUES (%s, %s)",
							   GetSQLValueString($filename, "text"),
							   GetSQLValueString($_POST['case_no'], "int"));
