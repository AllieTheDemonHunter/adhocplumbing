...
$query_red = sprintf("SELECT * FROM call_log INNER JOIN addresses using (addressID) WHERE call_status = 4 AND region = %s and calltype = 3 ORDER BY calltype ASC, logtime DESC", $myregionID);
...
mysql_select_db($database_adhocConn, $adhocConn);
$query_orange = sprintf("SELECT * FROM call_log INNER JOIN addresses using (addressID) WHERE call_status = 5 AND region = %s and calltype = 3 ORDER BY calltype ASC, logtime DESC", $myregionID);
...
mysql_select_db($database_adhocConn, $adhocConn);
$query_green = sprintf("SELECT * FROM call_log INNER JOIN addresses using (addressID) WHERE call_status = 6 AND region = %s and calltype = 3 ORDER BY calltype ASC, logtime DESC", $myregionID);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr><td bgcolor="#FFCC00" colspan="8" align="center" height="30">PATCHES</td>
	</tr>
                <?php 
				if ($totalRows_red > 0) {
				do { 
