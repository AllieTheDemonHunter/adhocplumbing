...
$query_recent = sprintf("SELECT * FROM call_log inner join addresses using (addressID) %s ORDER BY logID DESC", $extra);
...
<table width="100%" border="1" cellspacing="0" cellpadding="3">
  ...
  <tr>
      ...
      <td class="fineprint"><?php echo $row_recent['unitno']; ?> <?php echo $row_recent['complex']; ?><br>
        <?php echo $row_recent['streetno']; ?> <?php echo $row_recent['street']; ?><br>
        <?php
			mysql_select_db($database_adhocConn, $adhocConn);
			$query_sname = sprintf("SELECT * FROM suburbs WHERE suburbID = %s", $row_recent['suburb']);
